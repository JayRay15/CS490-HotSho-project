...
export default function InterviewAnalytics() {
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      {activeTab === "conversion" && <ConversionTab analytics={analytics} />}
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <MetricCard
          icon="🎯"
          label="Success Rate"
          value={`${conversionRates.successRate}%`}
          color="purple"
        />
        <MetricCard
          icon="⭐"
      ...
      <Card>
        ...
        <div className="space-y-3">
          <FunnelStage
            label="Scheduled"
            count={conversionRates.funnel.scheduled.count}
            percentage={conversionRates.funnel.scheduled.percentage}
            color="blue"
          />
          <FunnelStage
            label="Completed"
            count={conversionRates.funnel.completed.count}
            percentage={conversionRates.funnel.completed.percentage}
            color="green"
          />
          <FunnelStage
            label="Successful"
            count={conversionRates.funnel.successful.count}
            percentage={conversionRates.funnel.successful.percentage}
            color="purple"
          />
          <FunnelStage
            label="Offers"
            count={conversionRates.funnel.offers.count}
            percentage={conversionRates.funnel.offers.percentage}
            color="yellow"
          />
        </div>
      </Card>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div>
            ...
            <p className="text-2xl font-bold text-gray-900">
              {improvementTracking.recentPerformance.successRate}%
            </p>
            <p className="text-xs text-gray-500">
              {improvementTracking.recentPerformance.count} interviews
            </p>
          </div>
          <div>
            <p className="text-sm text-gray-600">Trend</p>
            <div className="flex items-center space-x-2">
              {improvementTracking.trend === "improving" && (
                <span className="text-2xl">📈</span>
              )}
              {improvementTracking.trend === "declining" && (
                <span className="text-2xl">📉</span>
              )}
              {improvementTracking.trend === "stable" && (
                <span className="text-2xl">➡️</span>
              )}
              <p className="text-lg font-semibold capitalize">{improvementTracking.trend}</p>
            </div>
            <p className="text-xs text-gray-500">
              {improvementTracking.improvementScore > 0 ? "+" : ""}
              {improvementTracking.improvementScore} points
            </p>
          </div>
          <div>
            <p className="text-sm text-gray-600">Practice Impact</p>
            <p className="text-lg font-semibold text-gray-900">
              {improvementTracking.mockSessionsCompleted} mock{improvementTracking.mockSessionsCompleted !== 1 ? "s" : ""}
            </p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <Card>
          ...
          <div className="space-y-4">
            <RateRow
              ...
              value={conversionRates.completionRate}
              description={`${conversionRates.completed} of ${conversionRates.scheduled} scheduled`}
            />
            <RateRow
              label="Success Rate"
              value={conversionRates.successRate}
              description={`${conversionRates.successful} successful outcomes`}
            />
            <RateRow
              label="Offer Rate"
              value={conversionRates.offerRate}
              description={`${conversionRates.offers} offers received`}
            />
            <RateRow
              label="Progression Rate"
              value={conversionRates.progressionRate}
              description="Moved to next interview round"
            />
          </div>
        </Card>
        ...
        <Card>
          ...
          <div className="space-y-4">
            <BenchmarkRow
              label="Success Rate"
              userValue={benchmarks.user.successRate}
              industryValue={benchmarks.industry.successRate}
              comparison={benchmarks.comparison.successRate}
            />
            <BenchmarkRow
              label="Offer Rate"
              userValue={benchmarks.user.offerRate}
              industryValue={benchmarks.industry.interviewToOfferRate}
              comparison={benchmarks.comparison.offerRate}
            />
              ...
              <div key={idx} className="border-b border-gray-200 pb-3 last:border-0">
                <div className="flex items-center justify-between mb-2">
                  <div>
                    ...
                    <span className="ml-2 text-sm text-gray-500">
                      ({format.total} interview{format.total !== 1 ? "s" : ""})
                    </span>
                  </div>
                ...
                <div className="w-full bg-gray-200 rounded-full h-2">
                  <div
                    ...
                    style={{ width: `${format.successRate}%` }}
                  />
                </div>
                <p className="text-xs text-gray-500 mt-1">
                  Avg duration: {format.avgDuration} minutes
                </p>
              </div>
            ))}
          </div>
                  ...
                  <tr key={idx}>
                    ...
                    <td className="px-4 py-3 text-sm">
                      <span className={`font-semibold ${
                        industry.successRate >= 50 ? 'text-green-600' : 
                        industry.successRate >= 30 ? 'text-yellow-600' : 
                        'text-red-600'
                      }`}>
      ...
      <Card>
        ...
        {insights.length > 0 ? (
          ...
          <p className="text-gray-600">
            Complete more interviews to generate strategic insights
          </p>
        )}
      </Card>
        ...
        <Card>
          ...
          <div className="space-y-3">
            <div>
              ...
              <p className="text-lg font-semibold text-gray-900">
                {improvementTracking.recentPerformance.period}
              </p>
            </div>
            <div>
              <p className="text-sm text-gray-600">Interviews Completed</p>
              <p className="text-lg font-semibold text-gray-900">
                {improvementTracking.recentPerformance.count}
              </p>
            </div>
            <div>
            ...
            {improvementTracking.recentPerformance.avgRating && (
              <div>
                <p className="text-sm text-gray-600">Average Rating</p>
                <p className="text-lg font-semibold text-gray-900">
                  {improvementTracking.recentPerformance.avgRating} / 5
                </p>
              </div>
            )}
          </div>
        ...
        <Card>
          ...
          <div className="space-y-3">
            <div>
              <p className="text-sm text-gray-600">Period</p>
              <p className="text-lg font-semibold text-gray-900">
                {improvementTracking.olderPerformance.period}
              </p>
            </div>
            <div>
              <p className="text-sm text-gray-600">Interviews Completed</p>
              <p className="text-lg font-semibold text-gray-900">
                {improvementTracking.olderPerformance.count}
              </p>
            </div>
            <div>
              <p className="text-sm text-gray-600">Success Rate</p>
              <p className="text-2xl font-bold text-gray-600">
                {improvementTracking.olderPerformance.successRate}%
              </p>
            </div>
            {improvementTracking.olderPerformance.avgRating && (
              <div>
                <p className="text-sm text-gray-600">Average Rating</p>
                <p className="text-lg font-semibold text-gray-900">
                  {improvementTracking.olderPerformance.avgRating} / 5
                </p>
              </div>
            )}
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="text-center p-4 bg-purple-50 rounded-lg">
            <p className="text-3xl font-bold text-purple-600">
              {improvementTracking.mockSessionsCompleted}
            </p>
          ...
          <div className="text-center p-4 bg-blue-50 rounded-lg">
            <p className="text-3xl font-bold text-blue-600 capitalize">
              {improvementTracking.trend}
            </p>
            <p className="text-sm text-gray-600 mt-1">Performance Trend</p>
          </div>
          <div className="text-center p-4 bg-green-50 rounded-lg">
            <p className={`text-3xl font-bold ${
              improvementTracking.improvementScore > 0 ? 'text-green-600' :
              improvementTracking.improvementScore < 0 ? 'text-red-600' :
              'text-gray-600'
            }`}>
              {improvementTracking.improvementScore > 0 ? '+' : ''}
              {improvementTracking.improvementScore}%
            </p>
        ...
        <div className="flex items-start space-x-3">
          ...
          <div>
            ...
            <p className="text-gray-700">
              Based on your interview performance data, here are targeted recommendations
              to improve your success rate and interview outcomes.
            </p>
          </div>
        </div>
      </Card>
  ...
  return (
    <div className="border-b border-gray-200 pb-3 last:border-0">
      ...
      <p className={`text-xs font-medium ${
        comparison.includes('above') ? 'text-green-600' :
        comparison.includes('below') ? 'text-red-600' :
        'text-gray-600'
      }`}>
    ...
    <div className="flex items-center space-x-3">
      ...
      <div className="flex-1">
        <div className="w-full bg-gray-200 rounded-full h-8 relative">
          <div
            className={`h-8 rounded-full ${color} transition-all duration-500`}
            style={{ width: `${percentage}%` }}
          />
...
function PerformanceItem({ type, successRate, total, avgRating, isStrength }) {
  return (
    <div className={`p-3 rounded-lg ${
      isStrength ? 'bg-green-50' : 'bg-orange-50'
    }`}>
